<?php
/******************************************
Nama File : kd_jnslembaga.php
Fungsi    : Setup kode jenis lembaga
Dibuat    :	
 Tgl.     : 
 Oleh     : FB
...
$sqltext = "select * from kd_jns_lembaga order by kode";
$query = mysql_query($sqltext, $db);

echo "<html>\n";
echo "<head>\n";
echo "<title>Kode Jenis Lembaga</title>\n";
...
echo "<tr>\n";
echo "<td colspan=2 class='judul1'>Kode Jenis Lembaga</td>\n";
echo "</tr>\n";
echo "</table>\n";

//echo "<h2><b>Kode Jenis Lembaga</b></h2><p>\n";
...
$i = 0;
echo "<form name='hapus_jnslembaga' method='post' action='post_kdproyek.php' target='isi'>\n";
while ($row = mysql_fetch_array($query)) {
    $i++;
    echo "<tr>\n";
    echo "<td class='isi2' width='5%'><input type='checkbox' name='cb_hapus[]' value=".$row["kode"]."></td>\n";
    echo "<td class='isi2' width='5%'><a href='kd_jnslembaga.php?pid=".$row["kode"]."#form'><img src='../gambar/pen.gif' width='17' height='17' border=0></a>&nbsp;</td>\n";
...
if ($i==0) {
    ...
    echo "<input type=hidden name=namaform value='hapusjnslembaga'>\n";
    echo "<tr>\n";
...
function form_edit_kode($kode,$nama,$singk) {
    echo "<p id='form'>\n";
    echo "<form name='jns_lembaga' method='post' action='post_kdproyek.php' target='isi'>\n";
    echo "<input type=hidden name=namaform value='jnslembaga'>\n";
    if (is_null($kode)) {
    ...
    echo "<table width='100%' border=0>\n";
    echo "<tr>\n";
    echo "<td class='judul1' colspan=2>Penambahan / Ubah Kode Jenis Lembaga </td>\n";
    echo "</tr>\n";
    echo "<tr>\n";
