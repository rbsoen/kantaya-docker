<?php
/******************************************
Nama File : ubah_jadwal.php
Fungsi    : Mengubah jadwal kegiatan proyek
Dibuat    :	
 Tgl.     : 
 Oleh     : FB
...
echo "<tr>\n";
echo "<td class='judul1'>Ubah Kegiatan Proyek</td>\n";
echo "</tr>\n";
echo "</table>\n";
...
echo "<td width='30%'>: <input type=text name='no_kegiatan' size=20 value=".$jdwlkgtn["no_kegiatan"]."></td>\n";
echo "</tr>\n";
echo "<tr>\n";
...
echo "<td colspan=3>: <input type=text name='nama_kegiatan' size=40 value='".$jdwlkgtn["nama_kegiatan"]."'></td>\n";
echo "</tr>\n";
echo "<tr>\n";
...
echo "<td width='30%'>: \n";
     ...
     if ($jdwlkgtn["jenis_kegiatan"] == 'swa') {
         ...
         echo "<option value='swa'>Swakelola</option>\n";
         echo "<option value='sub' selected>Sub Kontrak</option>\n";
     } else {
         echo "<option value='swa' selected>Swakelola</option>\n";
         echo "<option value='sub'>Sub Kontrak</option>\n";
     }
...
echo "<td width='30%'>: \n";
     ...
     while ($row = mysql_fetch_array($query)) {
         if ($row["kode_mitra"] == $jdwlkgtn["subkont_mitra"]) {
             echo "<option value='".$row["kode_mitra"]."' selected>".$row["nama_mitra"]."</option>\n";
         } else {
...
echo "<td colspan=3>: \n";
     list($y,$m,$d)=split("-",$jdwlkgtn["rcn_tgl_mulai"]);
...
echo "<td colspan=3>: \n";
     list($y,$m,$d)=split("-",$jdwlkgtn["rcn_tgl_selesai"]);
...
echo "<td width='20%'>Tgl Mulai</td>\n";
echo "<td colspan=3>: \n";
     list($y,$m,$d)=split("-",$jdwlkgtn["akt_tgl_mulai"]);
     echo "<input type=hidden name='akt_tgl_mulai'>\n";
     echo "<select name='akttglmulai'>\n";
     for ($i=1; $i<=31; $i++){
         if ($i == $d) {
             echo "<option value=$i selected> $i</option>\n";
         } else {
...
echo "<td width='20%'>Tgl Selesai</td>\n";
echo "<td colspan=3>: \n";
     list($y,$m,$d)=split("-",$jdwlkgtn["akt_tgl_selesai"]);
     echo "<input type=hidden name='akt_tgl_selesai'>\n";
     echo "<select name='akttglselesai'>\n";
     for ($i=1; $i<=31; $i++){
         if ($i == $d) {
             echo "<option value=$i selected> $i</option>\n";
         } else {
     ...
     echo "</select>\n";
     echo "<select name='aktblnselesai'>\n";
     for ($i=1; $i<=12; $i++){
         if ($i == $m) {
             echo "<option value=$i selected> ".namabulan("P",$i)."</option>\n";
         } else {
     ...
     echo "</select>\n";
     echo "<select name='aktthnselesai'>\n";
     for ($i=0; $i<=2; $i++){
         if (date("Y")-1+$i == $y) {
             echo "<option value=".(date("Y")-1+$i)." selected>".(date("Y")-1+$i)."</option>\n";
         } else {
...
echo "<tr>\n";
echo "<td width='20%'>&nbsp;</td>\n";
echo "<td colspan=3 valign='top' value=".$jdwlkgtn["keterangan"]."><textarea rows=5 cols=40 name='keterangan'></textarea></td>\n";
echo "</tr>\n";
echo "</table><p>\n";
...
while ($row = mysql_fetch_array($query)) {
    echo "<tr>\n";
    $where = "a.kode_proyek = '$p1' and a.no_kegiatan = '$p2' and a.kode_pengguna =".$row["kode_pengguna"];
    $checkqry = mysql_query("select count('x') from penugasan_personil a where $where",$db);
    $check = mysql_fetch_array($checkqry);
    if ($check[0] == 0) {
        echo "<td width='5%'><input type='checkbox' name='idpersonil[]' value=$i></td>\n";
        echo "<td width='30%'>".$row["nama_pengguna"]."</td>\n";
        echo "<td width='15%'>".$row["jabatan"]."</td>\n";
        echo "<td width='40%'><input type=text name='job[]' size=40></td>\n";
        echo "<td width='10%'><input type=text name='org_jam[]' size=5></td>\n";
        echo "<input type=hidden name='personil[]' value=".$row["kode_pengguna"].">\n";
    } else {
        $jobqry = mysql_query("select a.job, a.orang_jam FROM penugasan_personil a WHERE $where", $db);
        $job = mysql_fetch_array($jobqry);
        echo "<td width='5%'><input type='checkbox' checked name='idpersonil[]' value=$i></td>\n";
        echo "<td width='30%'>".$row["nama_pengguna"]."</td>\n";
        echo "<td width='15%'>".$row["jabatan"]."</td>\n";
        echo "<td width='40%'><input type=text name='job[]' size=20 value=".$job["job"]."></td>\n";
        echo "<td width='10%'><input type=text name='org_jam[]' size=5 value=".$job["orang_jam"]."></td>\n";
        echo "<input type=hidden name='personil[]' value=".$row["kode_pengguna"].">\n";
    }
    echo "</tr>\n";
    $i++;
}
echo "<input type=hidden name='jml_personil' value=$i>\n";
echo "</table>\n";
echo "<p>\n";
