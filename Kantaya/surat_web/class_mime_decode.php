...
class mime_decode {
	...
	global $mime_show_html,$sid,$lid,$ix,$folder;
		...
		for($i=0;$i<count($part);$i++) {
			...
			$have_file_name = ereg("name=",$headers["content-disposition"].$headers["content-type"]);

			if($rctype == "multipart/alternative")
				$msgbody = $this->build_alternative_body($ctype,$body);
			elseif($rctype == "text/plain" && !$have_file_name) {
				$body = $this->compile_body($body,$headers["content-transfer-encoding"]);
				$msgbody = $this->build_text_body($body);
			} elseif($rctype == "text/html" &&  !$have_file_name) {
				$body = $this->compile_body($body,$headers["content-transfer-encoding"]);
				if(!$mime_show_html) $body = $this->build_text_body(strip_tags($body));
				$msgbody = $body;
			} elseif($have_file_name) {
				...
				$msgbody = $this->process_message($header,$body);
		}
		return $msgbody;
	}
	...
	function make_link_clickable($text){
		$text = eregi_replace("(([[:alnum:]]+)://([[:alnum:]#?:/&=_.;,-]*))","<a class=autolink target=_blank href=\"\\1\">\\1</a>",$text);
		$text = eregi_replace("[^://](www\.[[:alnum:]#?:/&=_;,.-]*)","<a class=autolink target=_blank href=\"http://\\1\">\\1</a>",$text);
		$text = eregi_replace("([[:alnum:]_.-]+@[[:alnum:]_.-]+\.[[:alnum:]#?:/&=_;,.-]*)","<a class=autolink href=\"kirimsurat.php?mailto=\\1&nameto=\\1\">\\1</a>",$text);
		return $text;
	}
					...
					if($thispart != "") {
						...
						elseif($rctype == "text/plain" && !$have_file_name) {
							...
							if(!$mime_show_html)
								$body = $this->build_text_body(strip_tags($body));
							$msgbody = $body;
						} elseif($rctype == "multipart/related" && !$have_file_name) {
	...
	function get_mail_info($header) {
		...
		*/
		...
		if(eregi("([0-9]{1,2}[ ]+[A-Z]{3}[ ]+[0-9]{4}[ ]+[0-9]{1,2}:[0-9]{1,2}:[0-9]{1,2})",$received,$regs)) {
			//eg. Tue, 4 Sep 2001 16:22:31 
			$mydate = $regs[1];
	...
	function build_mime_date($mydate) {
		...
		//echo("$mydate: $hour, $min, $sec, $mon, $day, $year<br>");
